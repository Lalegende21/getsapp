...
export class CampusComponent implements OnInit {
  ...
  lastCenterTotal!: Center;
  lastCenterTotalRecently!: Center;
  centers: Center[] = [];
  centerFrequently: Center[] = [];
  filteredCenter: Array<Center> = [];
  ...
  centerFrequentlyTotal: number = 0;
  ...
  initializeModalVisibility() {
    this.isModalVisible = Array(this.centers.length).fill(false);
  }
  ...
  onOptionSelectedDelete(index: number, event: Event, id: number) {
    ...
    this.spinner.show();
    this.campusService.deleteCampusById(id).subscribe({
      next: () => {
        setTimeout(() => {
          this.spinner.hide();
        }, 2000);
  ...
  visibleData() {
    ...
    this.currentVisibleData = this.filteredCenter.slice(startIndex, endIndex);
  }

  nextPage() {
    const totalPages = Math.ceil(this.filteredCenter.length / this.pageSize);
  ...
  changePage(pageNumber: number) {
    if (
      pageNumber >= 1 &&
      pageNumber <= Math.ceil(this.filteredCenter.length / this.pageSize)
    ) {
      this.currentPage = pageNumber;
      this.visibleData();
    }
  ...
  //Recherche par nom de centre
  filterByName(searchItem: string) {
    this.filteredCenter = this.centers.filter((item) => {
      return item.name?.toLowerCase().includes(searchItem.toLowerCase());
    });
    this.visibleData(); // Met à jour les données visibles pour la pagination
  }

  //Recherche par localisation de centre
  filterByLocalisation(searchItem: string) {
    this.filteredCenter = this.centers.filter((item) => {
  ...
  sortData(column: string) {
    ...
    this.filteredCenter.sort((a, b) => {
      const valueA = a[column as keyof Center] as any;
      const valueB = b[column as keyof Center] as any;
  ...
  //Methode pour consulter un campus
  consulterCampus(id: number) {
    this.router.navigateByUrl('/dashboard/campus/' + id);
  }

  //Methode pour enregistrer un centre
  getAllCenter() {
    this.campusService.getAllCenter().subscribe({
      next: (response) => {
        this.centers = response;
        this.centerTotal = response.length;
        this.filteredCenter = this.centers;
        this.lastCenterTotal = response[0];
        this.visibleData();
      },
      error: (error) => {
        ...
        if (!errorMessage) {
          this.toastr.error(
            'Impossible de récupérer la liste des campus!'
          );
  ...
  //Methode pour enregistrer un centre
  getAllCenterFrequently() {
    this.campusService.getAllCenterFrequently().subscribe({
      next: (response) => {
        this.centerFrequently = response;
        this.centerFrequentlyTotal = response.length;
        this.lastCenterTotalRecently = response[0];
      },
      error: (error) => {
        // Vérifie si error.error existe
        if (!error.error) {
        ...
        if (!errorMessage) {
          this.toastr.error(
            'Impossible de récupérer la liste des campus ajoutés recemment!'
          );
        } else if (errorMessage === 'Access Denied') {
          this.toastr.error(
            "Vous n'êtes pas autorisé à consulter cette information."
